...
public class Server {
	...
	public Server() throws Exception {
		...
		while (true) {
			...
			case PLAY_REQUEST:
				...
				logln("recieved play request from client:- " + pPacket.username + " "
						+ dataPacket.getAddress().getHostAddress() + " " + dataPacket.getPort());

				if (rooms.isEmpty()) {
					Room room = new Room();
					User user = new User(pPacket.username, null, dataPacket.getAddress().getHostAddress(),
							dataPacket.getPort(), null);
					room.addUser(user);
			...
			case BULLET:
				...
				for (int i = 0; i < bulletPacket.users.size(); i++) {
					User currentUser = bulletPacket.users.get(i);
					sendData(bulletPacket, currentUser);
				}
	...
	public User findSender(DatagramPacket dataPacket, LinkedList<User> users) {
		for (int i = 0; i < users.size(); i++) {
			...
			if (currentUser.ipAddress.equals(dataPacket.getAddress().getHostAddress())
					&& currentUser.port == dataPacket.getPort()) {
				return currentUser;
			}
		}
	...
	public void sendData(Packet packet, User user) {
		try {
			DatagramPacket dataPacket = new DatagramPacket(packet.getData(), packet.length(),
					InetAddress.getByName(user.ipAddress), user.port);
			server.send(dataPacket);
		} catch (IOException e) {
			e.printStackTrace();
		}
